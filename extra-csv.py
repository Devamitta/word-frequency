...
df_class = pd.read_csv("../spreadsheets/class-vocab.csv", sep="\t", dtype= str)
df_class.fillna("", inplace=True)
...
# removing numbers class
df_class['Pāli2'] = df_class['Pāli1']
df_class['Pāli1'] = df_class['Pāli1'].str.replace('\d+', '')
df_class['Pāli1'] = df_class['Pāli1'].str.replace(' ', '')
...
# if headword from df_final_nid is in df_class, then delete whole row from df_final_nid

test1 = df_final_nid['Pāli1'].isin(df_class['Pāli1'])
test2 = df_final_nid['Pattern'].isin(df_class['Pattern'])
logix = test1 & test2

df_final_nid = df_final_nid.drop(df_final_nid[logix].index)
...
# save csv
df_final_nid.to_csv(f"../spreadsheets/add-class.csv", sep="\t", index=None)
